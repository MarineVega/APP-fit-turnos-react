...
*/
  ...
  const actividadesVisibles = Array.from(
    { length: visible },
    (_, i) =>
  ...
  useEffect(() => {
    ...
    if (
      onSeleccion &&
      actividadesVisibles[indiceSeleccionada] &&
      actividadesVisibles[indiceSeleccionada] !== null
    ) {
  ...
  const retroceder = () => {
    // Ahora es seguro porque sabemos que actividades.length > 0
    setIndiceInicio((prev) => (prev === 0 ? actividades.length - 1 : prev - 1));
  };
